...
import { ceramic, family, generateDid, loadDocumentByMetadata, PERSIST_CREATE_OPTS } from 'utils/ceramic'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    ceramic.did = did
    const mainDoc = await loadDocumentByMetadata(ceramic.did.id, family, [`wallet:${address}`], PERSIST_CREATE_OPTS)
    const chainDoc = await loadDocumentByMetadata(ceramic.did.id, family, [`wallet:${address}`, `chain:${body.chain}`], PERSIST_CREATE_OPTS)
