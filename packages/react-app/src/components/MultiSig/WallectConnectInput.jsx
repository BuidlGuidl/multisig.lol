...
const WalletConnectInput = ({ chainId, walletAddress, loadTransactionData, mainnetProvider, price }) => {
  // localstates
  ...
  useEffect(() => {
    if (walletConnectUri && walletAddress) {
  ...
  const subscribeToEvents = connector => {
    ...
    connector.on("call_request", (error, payload) => {
      console.log(`n-🔴 => connector.on => "call_request"`, "call_request");
      if (error) {
        throw error;
      }

      console.log("n-Event: call_request", payload);
  ...
  const parseCallRequest = payload => {
    console.log(`n-🔴 => parseCallRequest => payload`, payload);
    const callData = payload.params[0];
    console.log(`n-🔴 => parseCallRequest => callData`, callData);
